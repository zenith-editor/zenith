    ...
    pub fn updateLineWrap(
        ...
        if (start_line_is_mb) {
            ...
            while (iter.nextCodepointSliceUntil(next_line_offset)) |bytes| {
                const ccols = encoding.countCharCols(std.unicode.utf8Decode(bytes) catch unreachable);
        ...
        if (next_line_idx_new < next_line_idx) {
            ...
            std.mem.copyForwards(LineData, self.line_data.items[next_line_idx_new..new_len], self.line_data.items[next_line_idx..]);
        ...
        if (new_cont_lines.items.len > 0) {
            ...
            std.debug.assert(self.line_data.items[(start_line_idx + 1)..next_line_idx_new].len ==
                new_cont_lines.items.len);

            for (self.line_data.items[(start_line_idx + 1)..next_line_idx_new], new_cont_lines.items) |*line_data, cont_line_offset| {
                line_data.* = .{
                    .offset = cont_line_offset,
                    .line_no = line_no,
                    .flags = .{ .is_cont_line = true, .is_multibyte = start_line_is_mb },
                };
            }
        }
